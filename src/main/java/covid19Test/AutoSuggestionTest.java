...
public class AutoSuggestionTest {
  ...
  public void autoSuggestion() throws Exception {
    ...
    driver.findElement(By.cssSelector("input")).sendKeys("ma");
    Thread.sleep(2000);

    List<WebElement> list = driver.findElements(By.cssSelector("a:nth-child(1).Result"));
    for (int i = 0; i < list.size(); i++) {
      System.out.println(list.get(i).getText());
      String searchText = list.get(i).getText();

      if (searchText.equals("Maharashtra")) {
        list.get(i).click();
        break;
      }
      else{
        Assert.fail("Can not find maharashtra on sending ma keys");
